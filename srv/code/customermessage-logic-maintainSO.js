/**
 * 
 * @On(event = { "Action2" }, entity = "sunminJeon_1_23Srv.CustomerMessage")
...
module.exports = async function(request) {
	...
	try {
		...
		customerMessage = await SELECT.one.from('CustomerMessage').where({ ID });
		if (!customerMessage) {
	...
	return request.reply(200, { ServiceOrderId: soId });
}